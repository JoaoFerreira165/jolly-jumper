...
playState.prototype = {
    ...
    update: function () {
        ...
        let onGround = this.jolly.player.body.touching.down || this.jolly.player.body.blocked.down;
        if (!onGround) {
            this.jolly.player.body.velocity.x *= 0.99; // desacelera mais rápido

            if (this.jolly.player.body.velocity.x > 120) {
                this.jolly.player.body.velocity.x = 120;
            }
            if (this.jolly.player.body.velocity.x < -120) {
                this.jolly.player.body.velocity.x = -120;
            }
    ...
    playerVsPlatform: function () {
        ...
        this.jolly.player.body.velocity.y -= this.rnd.integerInRange(425, 600);
    },
